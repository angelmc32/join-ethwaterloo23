...
const AccountAbstractionProvider = ({ children }: { children: ReactNode }) => {
  ...
  // chain selected
  const [chainId, setChainId] = useState<number>(initialChain.id);
  ...
  const chain = supportedChains.find(({ id }) => id === chainId) ?? initialChain;
